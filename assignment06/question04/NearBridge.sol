...
contract NearBridge is INearBridge, AdminControlled {
    ...
    /// Deposit of `lockEthAmount` of ETH to be allowed to add light client blocks.
    ...
    /// Withdraw all deposited ETH and stop being allowed to add light client blocks.
    ///
    /// Cannot be performed if the sender was the last block submitter as their block can
    /// still be challenged.
    ...
    /// Challenges the specific signature in the current light client chain head.
    ///
    /// Discards the current chain head and sends half of the submitter's locked ETH to the
    /// challenger if successful.
    function challenge(address payable receiver, uint signatureIndex) external override pausable(PAUSED_CHALLENGE) {
        ...
        // Discard the current chain head.
        lastValidAt = 0;
        receiver.call{value: lockEthAmount / 2}("");
    }

    /// Checks the Ed25119 signature of the specified block producer in the current chain head and returns
    ...
    /// Initialises the light client with the block producers of the starting epoch.
    ...
    /// Initialises the light client with the current chain head.
    function initWithBlock(bytes memory data) public override onlyAdmin {
        ...
        // Set the light client to accept blocks from the next expected block producers.
    ...
    function bridgeState() public view returns (BridgeState memory res) {
        // If the challenge period of the chain head has not yet elapsed...
        if (block.timestamp < lastValidAt) {
            // Return the current confirmed block height and details on the current unconfirmed block.
            ...
            // Else, just return the height of the current trusted head.
            res.currentHeight = lastValidAt == 0 ? curHeight : untrustedHeight;
        }
    }

    /// Adds a new light client block as the chain head with some basic validation but optimistically
    /// accepts them without validating block producer signatures.
    function addLightClientBlock(bytes memory data) public override pausable(PAUSED_ADD_BLOCK) {
        ...
        unchecked {
            ...
            // Track the added block as the untrusted chain head.
            ...
            // Store the signatures of the untrusted block's block producers.
            ...
            // If this added block is from the next epoch, track the block producers
            // it specifies as those from the epoch after it.
            if (fromNextEpoch) {
    ...
    /// Sets the details of the block producers of the specified epoch.
    function setBlockProducers(NearDecoder.BlockProducer[] memory src, Epoch storage epoch) internal {
        ...
        epoch.numBPs = cnt;
        unchecked {
            // Store the public keys of all block producers for the specified epoch.
            for (uint i = 0; i < cnt; i++) {
                epoch.keys[i] = src[i].publicKey.k;
            }
            // Store the stake of each block producer for the specified epoch,
            // packing the stake of two block producers into one bytes32 slot.
            ...
            // Compute the stake threshold needed for a block in an epoch to be
            // considered valid.
