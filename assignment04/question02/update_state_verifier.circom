...
// for each proof, update 2**m transactions
    ...
    for (var i = 0; i < 2**m; i++) {
        ...
        // Check that this transaction exists within the provided transaction root.
        ...
        // Check that the sender exists within the account tree.
        ...
        // Check that the sender has sufficient tokens to send ie. will not underflow their balance.
        balanceFrom[i] - amount[i] <= balanceFrom[i]
        // Check that the receiver balance will not overflow when receiving the sender's tokens.
        balanceTo[i] + amount[i] >= balanceTo[i];

        // Check that the sender nonce is not at its max.
        ...
        // Update the sender's account state by removing their sent tokens and
        // incrementing their nonce.
        ...
        // Compute an intermediate root with the updated sender state and check that
        // it is consistent with the provided input.
        ...
        // Add the sent tokens to the receiver's balance if the
        // receiver address is not the withdrawal address.
        allLow[i] = AllLow(2);
        allLow[i].in[0] <== toX[i];
        allLow[i].in[1] <== toY[i];
        ...
        // Compute an intermediate root with the updated receiver state and check that
        // it is consistent with the provided input.
