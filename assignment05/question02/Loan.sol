...
/// A contract representing an instance of a loan between two parties.
contract Loan is ZkAssetMintable {
  ...
  /// Adds a sentinel to the lender approval map to indicate that the lender
  /// is requesting access to view the `Loan` notional.
  function requestAccess() public {
    lenderApprovals[msg.sender] = '0x';
  }

  /// Stores the viewing key for the loan notional on-chain encrypted with the lender's
  /// public key for them to view.
  function approveAccess(address _lender, bytes memory _sharedSecret) public {
    lenderApprovals[_lender] = _sharedSecret;
  }

  /// Marks the loan as settled and swaps the ownership of the loan and notional currency notes
  /// if it can be proven that the note swap was performed properly.
  function settleLoan(
  ...
  /// Withdraws the interest on this loan to the lender if it can be proven that the `Loan` balance is sufficent
  /// and that the withdrawal join-split was performed properly.
  ...
  /// Repays the `Loan` if the loan balance and the repayment swap from borrower to lender is validly proven.
  function repayLoan(
    bytes memory _proof1,
    bytes memory _proof2
  ) public {
  ...
  /// Marks the loan as defaulted if it can be proven that the interest accrued on the loan is now larger than
  /// the balance of the lender can withdraw.
